    ...
    <Link href={`/book/${book.id}`} className="block group">
      ...
      >
        ...
        >
          ...
          <div className="absolute top-2 right-2 text-lg" title={book.status}>
...
export default function ShelfClient({ books }: { books: Book[] }) {
  ...
  const filtered = books
    ...
    .sort((a, b) => {
      ...
      if (sort === 'oldest') return 0; // already in desc order from DB; oldest = reverse
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-4xl font-bold mb-1" style={{ color: '#2d1a0a', fontFamily: 'Georgia, serif' }}>
          My Reading Shelf
        </h1>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            {currentlyReading.map(book => (
              ...
              >
                <div
                  className="flex-shrink-0 w-12 h-16 rounded-lg flex items-center justify-center text-2xl shadow-sm overflow-hidden"
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6 items-start sm:items-center">
        <input
          ...
          className="px-4 py-2.5 rounded-xl text-sm flex-1 sm:max-w-64"
          style={{ background: '#fdf5e8', border: '1px solid #d4b896', color: '#2d1a0a' }}
        />
        <select
          value={sort}
        ...
        <div className="flex gap-2 ml-auto">
          <Link
            ...
            style={{ background: '#5c3d8f', color: '#fdf5e8' }}
          >
            🔍 Find Books
          </Link>
          <Link
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-xl mb-2 italic" style={{ color: '#4a2c17', fontFamily: 'Georgia, serif' }}>
            {books.length === 0 ? 'Your shelf is empty' : 'Nothing matches'}
          </p>
          <p className="mb-6 text-sm" style={{ color: '#8b5e3c' }}>
            {books.length === 0 ? 'search for books to add them to your shelf' : 'try a different filter or search'}
          </p>
          {books.length === 0 && (
            <Link
              href="/search"
